import datetime
from typing import Any, Callable, Dict, Generator, Union, cast
...
# The type_callable_map maps FieldDescriptor types to callables (e.g., to convert values).
TYPE_CALLABLE_MAP: Dict[int, Callable[[Any], Any]] = {
...
def repeated(type_callable: Callable[[Any], Any]) -> Callable[[Any], Any]:
    return lambda value_list: [type_callable(value) for value in value_list]


def enum_label_name(field: FieldDescriptor, value: Any, lowercase_enum_lables: bool = False) -> str:
...
def protobuf_to_dict(
    pb: Message,
    type_callable_map: Dict[int, Callable[[Any], Any]] = TYPE_CALLABLE_MAP,
    ...
    if including_default_value_fields:
        for field in pb.DESCRIPTOR.fields:
            ...
            if (
                field.label != FieldDescriptor.LABEL_REPEATED and field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE
...
REVERSE_TYPE_CALLABLE_MAP: Dict[int, Callable[[Any], Any]] = {}


def dict_to_protobuf(
    pb_klass_or_instance: Union[Message, type],
    values: Dict[str, Any],
    type_callable_map: Dict[int, Callable[[Any], Any]] = REVERSE_TYPE_CALLABLE_MAP,
    strict: bool = True,
    ignore_none: bool = False,
) -> Message:
    """Populates a protobuf model from a dictionary."""
    instance = pb_klass_or_instance if isinstance(pb_klass_or_instance, Message) else pb_klass_or_instance()
    return _dict_to_protobuf(instance, values, type_callable_map, strict, ignore_none)


def _get_field_mapping(pb: Message, dict_value: Dict[str, Any], strict: bool) -> list[tuple[FieldDescriptor, Any, Any]]:
    field_mapping = []
    ...
    for ext_num, ext_val in dict_value.get(EXTENSION_CONTAINER, {}).items():
        try:
            ...
            raise ValueError("Extension keys must be integers.") from e
        if ext_num not in pb._extensions_by_number:  # type: ignore
            if strict:
                raise KeyError(f"{pb} does not have an extension with number {key}. Perhaps you forgot to import it?")
            continue
...
def _dict_to_protobuf(
    ...
    for field, input_value, pb_value in fields:
        ...
        if field.is_extension:
            pb.Extensions[field] = input_value  # type: ignore
            continue
...
def _string_to_enum(field: FieldDescriptor, input_value: str, strict: bool = False) -> int:
    try:
        input_value = field.enum_type.values_by_name[input_value].number
    except KeyError as e:
        if strict:
            raise KeyError(f"`{input_value}` is not a valid value for field `{field.name}`") from e
        else:
            return _string_to_enum(field, input_value.upper(), strict=True)
    return cast(int, input_value)


def get_field_names_and_options(pb: Message) -> Generator[tuple[Any, Any, dict[Any, Any]], Any, None]:
    """
...
def validate_dict_for_required_pb_fields(pb: Message, dic: Dict[str, Any]) -> None:
    """
    Validate that the dictionary has all the required fields for creating a protobuf object
    from the pb class. If a field is missing, raise FieldsMissing.
    ...
    if missing_fields:
        raise FieldsMissing(f"Missing fields: {', '.join(missing_fields)}")
